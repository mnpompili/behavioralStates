function [spectrogram,t,f,phases] = helper_WaveletSpectrogram(lfp,varargin)
...
%     'resolution'  desired frequency resolution (sub-octaves per octave)
...
spectrogram = flipud((abs(wave)).^2);        % compute wavelet power spectrum
f = 1./flipud(scale(:));
if nargout>3,phases = wrap(atan2(imag(wave),real(wave)),2); end

% Downsample to desired temporal resolution
if ~isempty(step),
    t0 = t;
    t = (t0(1):step:t(end))';
    spectrogram = interp1(t0,spectrogram',t)';
    if nargout>3 && rem(step,mode(diff(t)))~=0,
        unwrapped = zeros(size(phases));
        for i=1:length(f), unwrapped(i,:) = unwrap(phases(i,:));end
        unwrapped = interp1(t0,unwrapped',t)';
        phases = wrap(unwrapped,2);
    end
end
